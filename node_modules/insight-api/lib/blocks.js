...
BlockController.prototype.getBlockReward = function(height) {
    var COIN = 100000000;
    var subsidy, k, b, c, ybnum, x0, x, e, r;
    if (height === 1) {
        subsidy = 50000000 * COIN;
    } else if (height === 2) {
        subsidy = 50000000 * COIN;
    } else if (height === 3) {
        subsidy = 60305704 * COIN;
    } else if (height >= 4 && height <= 780) {
        subsidy = 1 * COIN;
    } else {
        // S-Curve theory: Gompertz curve
        k = 10000000000; // Targrt 100 years ago
        b = 0.1;
        c = 2.0;
        ybnum = 365 * 24 * 60;
        x0 = -0.3;
        ...
        e = Math.exp(-c * x);
        r = -k * c * Math.log(b) * e * Math.pow(b, e) / ybnum / 10.0;
        if (r >= 10) {
            r = Math.floor(r);
        }
        subsidy = r * COIN;
    }
    subsidy = new BN(subsidy);
    return subsidy;
